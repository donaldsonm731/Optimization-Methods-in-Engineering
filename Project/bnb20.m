% This Branch is Bound Alg. Is not mine. I found it on mathworks.
% Link: https://www.mathworks.com/matlabcentral/fileexchange/95-bnb
% Author: Koert Kuipers

% There is always 'the guy' for the job
...
ub = [80; 80; 200; 200];
% Set settings
setts = [0]; % Leave as default
% Set options
options = optimset('Display','iter','TolX',1e-4,'maxSQPiter',Inf);
nonlc = @nonlcon1;
%% Run the BNB Code
[errmsg,Z,X,t,c,fail] = BNB20(funct,x0, xstat,lb,ub,A,b,Aeq,beq,nonlc,setts,options);




%%
function [errmsg,Z,X,t,c,fail] = BNB20(funct,x0,xstat,xl,xu,a,b,aeq,beq,nonlc,setts,opts,varargin)
...
%g3 = @(R,L) 1296000 - pi*R^2*L - 4/3*pi*R^3; % Volume constraint
%g1 = @(Ts,R)  0.0193*R - Ts; % Ratio between R and Ts
%g2 = @(Th,R) 0.0095*R-Th; % Ratio between R and Th
%g3 = @(R,L) 1296000 - pi*R^2*L - 4/3*pi*R^3; % Volume constraint
%g4 = @(L) L-240; % Length Constraint
c(1) = (0.0193*x(3))/0.0625 - x(1);
c(2) = (0.0095*x(3))/0.0625- x(2);
c(3) = 1296000 - pi*x(3)^2*x(4) - 4/3*pi*x(3)^3;
c(4) = x(4) - 240;
ceq = [];
...
function w = fun(x)
    w =  string(0.6224*(0.0625*x(1))*x(3)*x(4) + 1.7781*(0.0625*x(2))*x(3)^2 + 3.1661*(0.0625*x(1))^2*x(4)...
                                + 19.84*(0.0625*x(1))^2*x(3));
end
